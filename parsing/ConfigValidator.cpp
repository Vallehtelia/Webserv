...
// Extracts the location path from a "location" line
...
// Validates the allow_methods directive
...
// Stub validation methods for demonstration
...
bool ConfigValidator::validateRoot(const std::string& line) {
	...
	removeRootAndSemicolon(line);
	std::cout << "Root path: " << removeRootAndSemicolon(line) << std::endl;
	// Updated regex to allow valid path characters
	//std::regex rootRegex(R"(^\s*root\s+(.*?);\s*$)");
...
std::string ConfigValidator::removeRootAndSemicolon(const std::string& line) {
    // Regex to match "root" at the start and ";" at the end
    std::regex rootRegex(R"(^\s*root\s+(.*?);\s*$)");
    std::smatch match;

    // If the line matches the pattern, extract the path part
    if (std::regex_match(line, match, rootRegex) && match.size() > 1) {
        return match[1].str(); // Extract and return the path part
    }

    // Return the original line if it doesn't match
    return line;
}
...
// Validates a single server block
